...
export default function TransformPanel({ onTransform, isProcessing }) {
  const actions = [
    ...
    <div className="py-6 px-4 grid grid-cols-2 gap-3">
      {actions.map((action, index) => (
        <motion.button
          ...
          onClick={() => !isProcessing && onTransform(action.id)}
          disabled={isProcessing}
          className="group relative p-4 rounded-2xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 hover:border-white/10 transition-all text-left flex flex-col gap-3 disabled:opacity-40 disabled:cursor-not-allowed"
        >
