...
import { Image, Wand2, Download, Loader2, CheckCircle2, AlertCircle, Clock, Sparkles, FileText, Trash2, Plus, X } from "lucide-react";
...
function NodeShell({ children, color, status, selected, className = "" }) {
  const border = STATUS_BORDER[status || "idle"];
  return (
    <div className={`relative rounded-2xl border overflow-visible transition-all duration-200 min-w-[200px] max-w-[240px] ${border} ${selected ? `ring-2 ring-offset-0` : ""} ${className}`}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            {imageUrls.map((url, idx) => (
              <div key={idx} className="relative group rounded-lg overflow-hidden">
                ...
                <button onClick={() => handleRemoveImg(idx)}
      ...
      <div className="p-3 space-y-2">
        ...
        {data.inputPreview && (
          <div className="rounded-lg overflow-hidden">
            <img src={data.inputPreview} alt="Input" className="w-full h-14 object-cover" />
          </div>
        )}
      </div>
    </NodeShell>
          ...
          <div className="flex flex-wrap gap-1">
            {STYLES.map(s => (
              <button key={s} onClick={() => data.onChange?.({ style: s })}
                ...
                style={(data.style || "None") === s ? { background: "rgba(255,184,0,0.1)" } : { background: "transparent" }}>
                {s}
              </button>
            ))}
          </div>
    ...
    <NodeShell color={color} status={data.status} selected={selected}>
      ...
      <div className="p-3 space-y-2">
        {data.resultUrl ? (
          ...
          <div className="h-20 rounded-xl border border-dashed border-white/8 flex items-center justify-center" style={{ background: "rgba(78,205,196,0.04)" }}>
