...
import {
  ...
  Wand2, Type, Download, Sparkles, ArrowLeft, Upload, Grid3x3, CheckCircle2, X
} from "lucide-react";
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-[#0f0f0f] flex-shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#FF6B35] to-[#F72C25] flex items-center justify-center">
        ...
        <div className="flex items-center gap-2">
          {currentImage ? (
            <div className="flex items-center gap-2">
              <img src={currentImage.preview || currentImage.url} className="w-7 h-7 rounded-md object-cover border border-white/20" />
              <span className="text-xs text-white/50 hidden sm:inline truncate max-w-[120px]">{currentImage.name}</span>
              <button onClick={() => setCurrentImage(null)} className="text-white/30 hover:text-red-400 transition-colors">
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
          ) : (
            <span className="text-xs text-white/30 hidden sm:inline">No image loaded</span>
          )}
          <button
