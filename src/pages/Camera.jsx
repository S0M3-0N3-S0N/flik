  ...
  return (
    <div className="fixed inset-0 bg-black select-none" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      ...
      >
        ...
        {/* Zoom capsule */}
        {!photo && (
          <div className="absolute left-1/2 -translate-x-1/2" style={{ bottom: 60 }}>
              ...
              {zoomPresets.map(z => {
                ...
                return (
                  <motion.button key={z} whileTap={{ scale: 0.85 }} onClick={() => setZoomPreset(z)}
                    className={`w-8 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all ${isActive ? 'bg-white/20 text-white' : 'text-white/50'}`}
